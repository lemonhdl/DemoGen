...
from diffusion_policies.model_dp_umi.vision.timm_obs_encoder import TimmObsEncoder
...
class DiffusionUnetTimmPolicy(BaseImagePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], fixed_action_prefix: torch.Tensor=None) -> Dict[str, torch.Tensor]:
        ...
        ############################################################
        # NOTE: Do the same thing as in dataset
        for key, val in obs_dict.items():
            if key in self.rgb_keys:
                obs_dict[key] = val.float() / 255.
        ###########################################################
        ...
        # obs_key = "wrist_img"
        # print(obs_dict[obs_key].shape)
        # print("obs before normalize min, max, mean:", obs_dict[obs_key].max().cpu(), obs_dict[obs_key].min().cpu())
        # normalize input
        nobs = self.normalizer.normalize(obs_dict)
        # print("[eval] obs max, min:", obs_dict[obs_key].max().cpu(), obs_dict[obs_key].min().cpu())
        # print("[eval] nobs max, min:", nobs[obs_key].max().cpu(), nobs[obs_key].min().cpu())
        # nobs = self.normalizer.normalize(obs_dict)
        B = next(iter(nobs.values())).shape[0]

        # condition through global feature
        # nobs = self.obs_encoder.timm_transforms(nobs)
    ...
    def compute_loss(self, batch):
        # obs_key = "wrist_img"
        # print("[train] obs max, min:", batch['obs'][obs_key].max().cpu(), batch['obs'][obs_key].min().cpu())
        ...
        # print("[train] nobs max, min:", nobs[obs_key].max().cpu(), nobs[obs_key].min().cpu())
        # nobs = self.obs_encoder.timm_transforms(nobs)
        global_cond = self.obs_encoder(nobs)
